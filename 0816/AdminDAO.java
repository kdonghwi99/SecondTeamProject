...
//		return avolist;
		...
		try {
			...
			while (rs.next()) { // 조회되는 레코드가 있다면 VO객체를 생성하여 해당 레코드 값을 저장
				...
				avo.setAdmBirth(rs.getDate("adm_birth"));
				avo.setAdmPath(rs.getString("adm_path"));
				avo.setAdmType(rs.getString("adm_type"));
				avo.setAdmRegi(rs.getDate("adm_regi"));
	...
	public int totalCount(String admId, String admName) {
		int count = 0;
		try {
...
//	}
	...
	public boolean admUpdate(AdminVO avo) {
		...
		try {
			...
			pstmt.setString(2, avo.getAdmType());
...
//		String admId = "";
	...
	// 비밀번호 찾기
	public String admFpw(AdminVO avo) {

		query = " SELECT adm_pw FROM admin WHERE adm_name = ? AND adm_phone = ? AND adm_id = ? ";
		try {	                                          //+ 해서 바로 변수명으로 불러올 수 있음
					
			pstmt = con.prepareStatement(query);				
			pstmt.setString(1, avo.getAdmName());								// 3. 데이터 바인딩
			pstmt.setString(2, avo.getAdmPhone());								// 3. 데이터 바인딩
			pstmt.setString(3, avo.getAdmId());								// 3. 데이터 바인딩
			rs = pstmt.executeQuery();							// 4. 쿼리문 실행
			if(rs.next()) {
				  return rs.getString("adm_pw");
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}	finally {
