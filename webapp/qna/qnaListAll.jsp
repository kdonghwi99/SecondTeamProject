...
<main role="main">
	...
	<div class="col-8">
		...
		<c:set var="type"	value="${empty param.category ? '전체보기' : param.category}" />
...
<%--					<td class="text-center"><a href="${qViewLink }">${qvo.qnaTitle }</a></td> --%>
 					<td>
 					<c:choose>
<%--				sid가 null이 아니라면 해당 링크로 이동 --%>
                    <c:when test="${not empty sid}">
                       	<c:if test="${sid eq 'admin' || sid eq qvo.admId}">
							<a href="${qViewLink }">${qvo.qnaTitle }</a>
						</c:if>
						<c:if test="${sid ne 'admin' && sid ne qvo.admId}">
							<a onclick="alert('문의글은 작성자 및 관리자만 조회할 수 있습니다.')">${qvo.qnaTitle }</a>
						</c:if>
                    </c:when>
 <%--               sid가 null인 경우, alert 띄우기 --%>
                    <c:otherwise>
							<a onclick="alert('로그인 후 이용해주세요.')">${qvo.qnaTitle }</a>
                    </c:otherwise>
                    </c:choose>
                    </td>         
				...
				<ul class="pagination justify-content-center custom-pagination">
					...
					<c:if test="${prev }">
					<li class="page-item ">
						<a href="./list.do?pageNum=${start-1 }&category=${category }" class="page-link">&laquo;이전</a> 
					</c:if>
					
					<%-- 페이지 번호 버튼 --%>
					<c:forEach begin="${start }" end="${end }" var="i">
						<c:url var="link" value="./list.do?pageNum=${i }&category=${category }"/>		<%-- pageNum에 해당하는 페이지로 넘어가게 하는 링크를 변수에 담음 --%>
					...
					<c:if test="${next }">
					<li class="page-item">
						<a href="./list.do?pageNum=${end+1 }&category=${category }" class="page-link">다음&raquo;</a>
					</c:if>
				</ul>
			</div>
		</c:if><!-- END 게시물이 있는 경우 -->
		<!-- session.sid가 있을 때에만 문의하기 버튼 보여주기 -->
 		<c:if test="${not empty sid}">  
		<div class="text-right">
			<a href="./WriteForm.do" class="btn btn-success btn-sm" id="RegiBtn">  문의하기  </a><br><br><br>
		</div>
 		</c:if>
